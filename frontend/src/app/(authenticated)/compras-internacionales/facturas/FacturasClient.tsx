...
interface Tarjeta {
  ...
  recibo?: { guia: string | null; factura: string | null; lineas: number; cajas: number; pendientes: number };
...
export function FacturasClient({ etaConfig }: { etaConfig: EtaConfig }) {
  ...
  const recargarSerie = useCallback(async () => {
    try {
      const r = await fetch('/api/compras-internacionales/reyma/factura/cargar');
      if (!r.ok) return;
  ...
  const subir = useCallback(async (archivos: FileList | File[]) => {
    ...
    await Promise.all(lista.map(async (archivo, i) => {
      ...
      try {
        const r = await fetch('/api/compras-internacionales/reyma/factura/extraer', { method: 'POST', body: fd });
  ...
  const guardar = useCallback(async (t: Tarjeta) => {
    ...
    try {
      ...
      actualizar(t.id, {
        estado: 'cargada',
        recibo: {
          guia: j.guia, factura: j.factura, lineas: j.lineas, cajas: j.cajas,
