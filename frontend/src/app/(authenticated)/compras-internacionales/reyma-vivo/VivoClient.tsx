...
export function VivoClient({ modelo = 'reyma' }: { modelo?: string } = {}) {
  ...
  const load = useCallback(() => {
    fetch(`/api/compras-internacionales/reyma?modelo=${encodeURIComponent(modelo)}`)
      .then(async (r) => {
              ...
              <button
                onClick={async () => {
                  ...
                  for (const [codigo, cajas] of Object.entries(proyEdits)) {
                    const e = await postJson('/api/compras-internacionales/reyma/proyeccion', { codigo, cajas });
      ...
      {tab === 'modelo' && (
        <TabModelo
          ...
          onClearOverride={async (codigo) => {
            const e = await postJson('/api/compras-internacionales/reyma/proyeccion', { codigo, cajas: null });
            if (!e) load();
...
function TabModelo({
  ...
  const guardarPrecio = async (codigo: string, actual: number, nuevo: number) => {
    ...
    const e = await postJson('/api/compras-internacionales/reyma/precio', { codigo, precio: nuevo });
    if (!e) onSaved();
...
function PanelConciliacion({
  ...
  const correr = async () => {
    setOcupado(true);
    const err = await postJson('/api/compras-internacionales/reyma/conciliacion', { accion: 'ejecutar', mes });
    setOcupado(false);
  ...
  const decidir = async (x: Excepcion, odooFactura: string, estado: 'confirmado' | 'rechazado') => {
    setOcupado(true);
    const err = await postJson('/api/compras-internacionales/reyma/conciliacion', {
        ...
        {plan && (
          <button
            onClick={async () => {
              ...
              const e = await postJson('/api/compras-internacionales/reyma/plan', {
                semana: proximoLunes(),
                payload: payloadPlan,
              });
              if (!e) onSaved();
    ...
    <>
      <div className="bg-white rounded-lg border border-slate-200 p-4 mb-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
          ...
          <button
            onClick={async () => {
              const e = await postJson('/api/compras-internacionales/reyma/nc-config', {
                ...
                <tr key={`${t.poName}-${t.codigo}-${i}`} className="border-t border-slate-100">
                  ...
                  <td className={TD}>
                    <span className="inline-flex items-center gap-1">
                      ...
                      <button
                        onClick={async () => {
                          const e = await postJson('/api/compras-internacionales/reyma/furgon-nota', {
                            poName: t.poName,
                            eta: edit.eta || null,
