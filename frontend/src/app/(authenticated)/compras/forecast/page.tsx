  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        <label className="text-sm text-gray-600 ml-2" htmlFor="filter-metrica">Métrica:</label>
        <select
          id="filter-metrica"
          value={metricFilter}
          onChange={(e) => setMetricFilter(e.target.value as '' | 'ventas' | 'compras_ordenadas' | 'compras_recibidas' | 'recomendado')}
          className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white"
        >
        ...
        <label className="text-sm text-gray-600 ml-2" htmlFor="filter-urgencia">Urgencia:</label>
        <select
          id="filter-urgencia"
          value={urgencyFilter}
          onChange={(e) => setUrgencyFilter(e.target.value as 'all' | 'urgent' | 'plan')}
          className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white"
        >
          <option value="all">Todos</option>
          <option value="urgent">Pedir ahora{urgentSkus.size > 0 ? ` (${urgentSkus.size})` : ''}</option>
          <option value="plan">Planificar</option>
        </select>

        <label className="text-sm text-gray-600 ml-2" htmlFor="filter-ordenar">Ordenar:</label>
        <select
          id="filter-ordenar"
          value={sortByFurgones ? 'furgones' : 'default'}
          onChange={(e) => setSortByFurgones(e.target.value === 'furgones')}
          className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white"
        >
      ...
      {/*
        ...
        Datos completos so the filter row is pure noise. Visible rows still
        pass through the tierFilter state above (defaults to all three tiers
        active). Restore this block + the openTip state + setTierFilter
        destructuring if the stoplight ever shows AMBER/RED again.
      ...
      <div className="flex gap-2 items-center flex-wrap">
        ...
        {(['green', 'amber', 'red'] as CompletenessTier[]).map((tier) => {
          ...
          const tipText: Record<CompletenessTier, string> = {
            green: '15 o 16 meses del período de entrenamiento con al menos una OC confirmada. Calidad de datos suficiente para forecast confiable.',
            amber: 'Entre 3 y 14 meses con OC confirmadas. Historial parcial.',
