...
import { Gauge, Flame, Snowflake, Minus, CheckCircle } from 'lucide-react';
  ...
  const warehouses = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      if (existing) {
        ...
        seen.set(r.warehouse_id, { id: r.warehouse_id, code: r.warehouse_code, name: r.warehouse_name, count: 1 });
      }
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Cuántos días de stock tenés por producto y bodega, según la demanda de los últimos 30 días.
          </p>
          {snapshotLabel && (
            <p className="text-xs text-gray-400 mt-1">
              Snapshot al {snapshotLabel}. La próxima versión refresca diariamente desde Odoo.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatusCard label="Hot (urgente)" count={counts.hot} color="text-red-600" active={statusFilter === 'hot'} onClick={() => setStatusFilter(statusFilter === 'hot' ? 'all' : 'hot')} />
        <StatusCard label="OK" count={counts.ok} color="text-green-600" active={statusFilter === 'ok'} onClick={() => setStatusFilter(statusFilter === 'ok' ? 'all' : 'ok')} />
        <StatusCard label="Hold (sobrante)" count={counts.hold} color="text-blue-600" active={statusFilter === 'hold'} onClick={() => setStatusFilter(statusFilter === 'hold' ? 'all' : 'hold')} />
        <StatusCard label="Sin demanda" count={counts.no_demand} color="text-gray-600" active={statusFilter === 'no_demand'} onClick={() => setStatusFilter(statusFilter === 'no_demand' ? 'all' : 'no_demand')} />
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium text-gray-500">Demanda/día</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">Cargando datos…</td></tr>
              ) : error ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-red-500">No se pudieron cargar los datos.</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">No hay resultados para los filtros seleccionados.</td></tr>
              ) : (
                filtered.slice(0, PAGE_SIZE).map((r) => {
                  ...
                  return (
                    <tr key={`${r.product_id}-${r.warehouse_id}`} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium text-gray-900 max-w-xs truncate" title={r.product_name}>
                        {r.product_name}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right text-gray-900 font-medium">
                        {r.days_of_supply === null ? '—' : r.days_of_supply > 999 ? '999+' : formatNumber(r.days_of_supply, 1)}
                      </td>
...
function StatusCard({ label, count, color, active, onClick }: StatusCardProps) {
  return (
    <button
      onClick={onClick}
      className={`text-left bg-white rounded-xl border p-5 transition-colors ${active ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-gray-200 hover:border-gray-300'}`}
    >
      <p className="text-sm text-gray-500">{label}</p>
      <p className={`text-3xl font-bold mt-1 ${color}`}>{count.toLocaleString('es-GT')}</p>
    </button>
  );
}
