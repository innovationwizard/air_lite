...
/**
 * POST /api/compras-internacionales/reyma/factura/cargar — paso 2 de la carga (A12).
...
export async function POST(request: Request) {
  ...
  interface RetenidaClaveSinMapa {
    tipo: string;
    ...
    factura: string;
    fecha: string;
    observ_destino: string;
  }
  const parse = (staging.parse ?? {}) as {
    cuadra?: boolean;
    errores?: string[];
    retenidas?: RetenidaClaveSinMapa[];
  ...
  // 2026-09-09: una factura puede no tener NINGUNA línea con clave conocida
  // (REYMA metió un producto nuevo en un pedido chico) — eso ya no es un
  // callejón sin salida: se pone TODO en cuarentena en vez de rechazar la
  // factura completa. Sólo es un error real si no hay absolutamente nada que
  // hacer con el documento.
  const pendientesElegibles = (parse.retenidas ?? []).filter((r) => r.tipo === 'clave_sin_mapa');
  if (filas.length === 0 && pendientesElegibles.length === 0) {
  ...
  // Claves sin mapa (2026-09-09): la línea NO se pierde — queda en cuarentena
  // para que Alexis la resuelva en /compras-internacionales/facturas/pendientes, cuando
  // pueda, no en este momento. `staging.estado` se marca 'cargada' RECIÉN
  // DESPUÉS de que esto escriba: si falla, la puerta de arriba («ya se
  // cargó») no bloquea un reintento — el upsert de `filas` de arriba ya es
  // idempotente (onConflict), y este insert también lo es (UNIQUE en la
  // tabla), así que reintentar el mismo ticket es seguro.
  if (pendientesElegibles.length > 0) {
    const aPendiente = pendientesElegibles.map((r) => ({
      clave: r.identificador,
      descripcion: r.descripcion,
      guia: staging.guia,
      factura: r.factura,
      folio_fiscal: r.folio_fiscal,
      archivo: r.archivo,
      fecha: r.fecha,
      observ_destino: r.observ_destino,
      destino,
      eta,
      cantidad_cfdi: r.cantidad,
      unidad: r.unidad,
      bultos: r.bultos,
      importe: r.importe,
      precio_unitario: r.precio_unitario,
      motivo: r.motivo,
      autor: procedencia.slice(0, 500),
    }));
    const { error: errPendiente } = await service
      .from('reyma_factura_pendiente')
      .upsert(aPendiente, { onConflict: 'folio_fiscal,clave', ignoreDuplicates: true });
    if (errPendiente) {
  ...
  return NextResponse.json({
    ...
    pendientes: pendientesElegibles.length,
  });
}
