...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc('rpc_oa_supplier_semaphore', {
      p_supplier_ids,
      p_warehouse_id,
    });
    ...
    console.error('OA supplier semaphore error:', error);
    return NextResponse.json(
      { error: 'Error al obtener semáforo de proveedores' },
      { status: 500 },
    );
  }
}
