-- ============================================================================
-- Fix: Drop old v1 function overloads that conflict with v2 signatures
-- The v1 functions had fewer parameters. CREATE OR REPLACE in v2 created
-- new overloaded versions instead of replacing, causing Supabase RPC
-- "Could not choose best candidate" errors.
-- ============================================================================

-- Drop ALL existing versions — both v1 (single param) and v2 (two params)
-- to eliminate overloads and fix TEXT vs VARCHAR mismatches
DROP FUNCTION IF EXISTS rpc_oa_net_inventory(INT[]);
DROP FUNCTION IF EXISTS rpc_oa_net_inventory(INT[], INT);
DROP FUNCTION IF EXISTS rpc_oa_hot_list(INT[]);
DROP FUNCTION IF EXISTS rpc_oa_hot_list(INT[], INT);
DROP FUNCTION IF EXISTS rpc_oa_hold_list(INT[]);
DROP FUNCTION IF EXISTS rpc_oa_hold_list(INT[], INT);
DROP FUNCTION IF EXISTS rpc_oa_supplier_semaphore(INT[]);
DROP FUNCTION IF EXISTS rpc_oa_supplier_semaphore(INT[], INT);
DROP FUNCTION IF EXISTS rpc_oa_reception_saturation(DATE);
DROP FUNCTION IF EXISTS rpc_oa_reception_saturation(DATE, INT);
DROP FUNCTION IF EXISTS rpc_oa_warehouse_space(INT);
DROP FUNCTION IF EXISTS rpc_oa_alerts_summary();
DROP FUNCTION IF EXISTS rpc_oa_compliance(INT);
DROP FUNCTION IF EXISTS rpc_oa_global_compliance(INT);

-- Also fix VARCHAR vs TEXT casting in the v2 functions.
-- PostgreSQL CASE expressions return TEXT, but function signatures declare VARCHAR.
-- Solution: explicitly cast CASE results to VARCHAR.

-- Fix rpc_oa_net_inventory: cast buffer_status and supplier_origin
...
BEGIN
  ...
  SELECT
    ...
    (CASE
      ...
      ELSE 'excess'
    END)::VARCHAR AS buffer_status,
    COALESCE(p.is_export, false) AS is_export,
    p.supplier_origin::VARCHAR
  FROM products p
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE id.product_id = p.id
      ...
      AND (p_warehouse_id IS NULL OR id.warehouse_id = p_warehouse_id)
  ) inv ON true
  LEFT JOIN LATERAL (
...
BEGIN
  ...
  SELECT
    ...
    ni.is_export,
    ni.supplier_origin
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
...
BEGIN
  ...
  SELECT
    ...
    ni.is_export,
    ni.supplier_origin,
    NOT COALESCE(ni.is_export, false) AS cancellable
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
...
-- Fix rpc_oa_supplier_semaphore: cast semaphore and semaphore_reason
...
BEGIN
  ...
  SELECT
    ...
    (CASE
      ...
      ELSE 'hold'
    END)::VARCHAR AS semaphore,
    (CASE
      WHEN ni.days_of_supply < v_min_stock_days
        THEN 'Necesidad de Ampliacion de OA'
      WHEN ni.days_of_supply < 5
        THEN 'Sugerencia de adelantar despachos de la proxima semana'
      WHEN ni.days_of_supply <= v_max_stock_days
        THEN 'Orden Abierta fluyendo segun lo pactado'
      ELSE CASE
        WHEN COALESCE(ni.is_export, false)
          THEN 'Exceso de inventario — Exportacion: no se puede pausar despacho en transito'
          ELSE 'Detener despachos — exceso de inventario sobre buffer de seguridad'
        END
    END)::VARCHAR AS semaphore_reason,
    ni.is_export,
    ni.supplier_origin
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
  ORDER BY
...
-- Fix rpc_oa_warehouse_space: cast alert_level
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT SUM(id.quantity_on_hand * COALESCE(p.volume_m3, 0)) AS total_m3
    FROM inventory_daily id
  ...
  LEFT JOIN LATERAL (
    SELECT SUM((pol.quantity - COALESCE(pol.received_qty, 0)) * COALESCE(p.volume_m3, 0)) AS total_m3
    FROM purchase_order_lines pol
    JOIN purchase_orders po ON po.id = pol.order_id
    JOIN products p ON p.id = pol.product_id
    WHERE po.state IN ('purchase', 'locked')
      AND (pol.quantity - COALESCE(pol.received_qty, 0)) > 0
      AND p.volume_m3 IS NOT NULL AND p.volume_m3 > 0
  ) inc ON true
  LEFT JOIN LATERAL (
...
-- Fix rpc_oa_reception_saturation: cast warehouse_name
...
BEGIN
  ...
  WITH target_warehouse AS (
    SELECT
      w.id AS wid,
      w.name::VARCHAR AS wname,
      COALESCE(wc.num_docks, 1) AS num_docks,
      EXTRACT(HOUR FROM (COALESCE(wc.working_hours_end, '00:00'::TIME) - COALESCE(wc.working_hours_start, '06:00'::TIME))
        + CASE WHEN wc.working_hours_end <= wc.working_hours_start THEN INTERVAL '24 hours' ELSE INTERVAL '0' END
      ) AS work_hours
    FROM warehouses w
    LEFT JOIN warehouse_config wc ON wc.warehouse_id = w.id
  ...
  day_schedule AS (
    ...
    CROSS JOIN target_warehouse tw
    WHERE rs.scheduled_date = p_date
      AND rs.status NOT IN ('cancelled', 'rescheduled')
      AND (p_warehouse_id IS NULL OR rs.warehouse_id = p_warehouse_id)
    GROUP BY rs.scheduled_date, tw.wid, tw.wname, tw.num_docks, tw.work_hours
  )
  SELECT
    ds.scheduled_date,
    ds.truck_count::INT AS total_trucks,
    ds.total_hours AS total_unload_hours,
    (ds.num_docks * ds.work_hours)::NUMERIC AS available_dock_hours,
    CASE
      WHEN (ds.num_docks * ds.work_hours) > 0
      THEN ROUND((ds.total_hours / (ds.num_docks * ds.work_hours)) * 100, 1)
      ELSE 0
    END AS saturation_pct,
    ds.total_hours > (ds.num_docks * ds.work_hours) AS is_saturated,
    ds.wname AS warehouse_name,
...
-- Fix rpc_oa_alerts_summary: cast all varchar results
CREATE OR REPLACE FUNCTION rpc_oa_alerts_summary()
RETURNS TABLE(
  hot_count BIGINT,
  hold_count BIGINT,
  hold_export_count BIGINT,
  reception_saturated BOOLEAN,
  reception_trucks_today INT,
  warehouse_alerts JSONB
) AS $$
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(*) FROM rpc_oa_hot_list(NULL, NULL))::BIGINT AS hot_count,
    (SELECT COUNT(*) FROM rpc_oa_hold_list(NULL, NULL))::BIGINT AS hold_count,
    (SELECT COUNT(*) FROM rpc_oa_hold_list(NULL, NULL) h WHERE h.is_export = true)::BIGINT AS hold_export_count,
    COALESCE((SELECT bool_or(rs.is_saturated) FROM rpc_oa_reception_saturation(CURRENT_DATE, NULL) rs), false) AS reception_saturated,
    COALESCE((SELECT SUM(rs.total_trucks) FROM rpc_oa_reception_saturation(CURRENT_DATE, NULL) rs), 0)::INT AS reception_trucks_today,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'warehouse_name', ws.warehouse_name,
        'alert_level', ws.alert_level,
        'saturation_pct', ws.saturation_pct
      )) FROM rpc_oa_warehouse_space(NULL) ws WHERE ws.alert_level IN ('amarillo', 'rojo')),
      '[]'::JSONB
    ) AS warehouse_alerts;
END;
$$ LANGUAGE plpgsql;
...
-- Re-create compliance RPCs (dropped above to clean up)
