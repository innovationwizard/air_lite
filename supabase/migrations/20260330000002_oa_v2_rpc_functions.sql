-- ============================================================================
-- OA Module v2 — Updated & New RPC Functions
-- Multi-warehouse support, export protection, dynamic warehouse space
-- ============================================================================


-- ============================================================================
-- 1. WAREHOUSE SPACE — Dynamic capacity calculation per warehouse
-- Formula: Occupied = SUM(qty_on_hand * product.volume_m3)
--          Available = max_capacity - Occupied
--          Incoming = SUM(transit_qty * product.volume_m3)
--          Post-Arrival = Available - Incoming
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_warehouse_space(
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  warehouse_id INT,
  warehouse_name VARCHAR,
  max_capacity_m3 NUMERIC,
  occupied_m3 NUMERIC,
  incoming_m3 NUMERIC,
  available_m3 NUMERIC,
  post_arrival_m3 NUMERIC,
  saturation_pct NUMERIC,
  products_without_volume BIGINT,
  alert_level VARCHAR
) AS $$
BEGIN
  RETURN QUERY
  SELECT
    w.id AS warehouse_id,
    w.name AS warehouse_name,
    COALESCE(wc.max_capacity_m3, 0) AS max_capacity_m3,

    -- Occupied: current inventory * product volume
    COALESCE(occ.total_m3, 0) AS occupied_m3,

    -- Incoming: confirmed transits * product volume
    COALESCE(inc.total_m3, 0) AS incoming_m3,

    -- Available
    COALESCE(wc.max_capacity_m3, 0) - COALESCE(occ.total_m3, 0) AS available_m3,

    -- Post-arrival
    COALESCE(wc.max_capacity_m3, 0) - COALESCE(occ.total_m3, 0) - COALESCE(inc.total_m3, 0) AS post_arrival_m3,

    -- Saturation %
    CASE
      WHEN COALESCE(wc.max_capacity_m3, 0) > 0
      THEN ROUND((COALESCE(occ.total_m3, 0) / wc.max_capacity_m3) * 100, 1)
      ELSE 0
    END AS saturation_pct,

    -- Products without volume data
    COALESCE(no_vol.cnt, 0) AS products_without_volume,

    -- Alert level
    CASE
      WHEN COALESCE(wc.max_capacity_m3, 0) = 0 THEN 'sin_configurar'
      WHEN (COALESCE(occ.total_m3, 0) + COALESCE(inc.total_m3, 0)) > wc.max_capacity_m3 THEN 'rojo'
      WHEN COALESCE(occ.total_m3, 0) / NULLIF(wc.max_capacity_m3, 0) > 0.95 THEN 'rojo'
      WHEN COALESCE(occ.total_m3, 0) / NULLIF(wc.max_capacity_m3, 0) > 0.80 THEN 'amarillo'
      ELSE 'verde'
    END AS alert_level

  FROM warehouses w
  LEFT JOIN warehouse_config wc ON wc.warehouse_id = w.id
  -- Occupied m3
  LEFT JOIN LATERAL (
    SELECT SUM(id.quantity_on_hand * COALESCE(p.volume_m3, 0)) AS total_m3
    FROM inventory_daily id
    JOIN products p ON p.id = id.product_id
    WHERE id.warehouse_id = w.id
      AND id.snapshot_date = (SELECT MAX(snapshot_date) FROM inventory_daily)
      AND p.volume_m3 IS NOT NULL
      AND p.volume_m3 > 0
  ) occ ON true
  -- Incoming m3
  LEFT JOIN LATERAL (
    SELECT SUM((pol.quantity - COALESCE(pol.received_qty, 0)) * COALESCE(p.volume_m3, 0)) AS total_m3
    FROM purchase_order_lines pol
    JOIN purchase_orders po ON po.id = pol.order_id
    JOIN products p ON p.id = pol.product_id
    WHERE po.state IN ('purchase', 'locked')
      AND (pol.quantity - COALESCE(pol.received_qty, 0)) > 0
      AND p.volume_m3 IS NOT NULL
      AND p.volume_m3 > 0
  ) inc ON true
  -- Products in this warehouse without volume
  LEFT JOIN LATERAL (
    SELECT COUNT(DISTINCT id.product_id) AS cnt
    FROM inventory_daily id
    JOIN products p ON p.id = id.product_id
    WHERE id.warehouse_id = w.id
      AND id.snapshot_date = (SELECT MAX(snapshot_date) FROM inventory_daily)
      AND id.quantity_on_hand > 0
      AND (p.volume_m3 IS NULL OR p.volume_m3 = 0)
  ) no_vol ON true
  WHERE w.is_active = true
    AND w.id IN (1, 2, 3, 4) -- Only operational warehouses (Central, Zona 11, Peten, Zacapa)
    AND (p_warehouse_id IS NULL OR w.id = p_warehouse_id)
  ORDER BY w.id;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 2. UPDATED NET INVENTORY — Now supports warehouse filter
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_net_inventory(
  p_supplier_ids INT[] DEFAULT NULL,
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  product_id INT,
  product_name VARCHAR,
  sku VARCHAR,
  category VARCHAR,
  supplier_id INT,
  supplier_name VARCHAR,
  physical_inventory NUMERIC,
  confirmed_transits NUMERIC,
  pending_customer_orders NUMERIC,
  net_inventory NUMERIC,
  avg_daily_demand NUMERIC,
  days_of_supply NUMERIC,
  buffer_status VARCHAR,
  is_export BOOLEAN,
  supplier_origin VARCHAR
) AS $$
DECLARE
  v_min_stock_days INT;
  v_max_stock_days INT;
BEGIN
  SELECT COALESCE((value)::INT, 3) INTO v_min_stock_days
  FROM app_settings WHERE key = 'oa_min_stock_days' LIMIT 1;

  SELECT COALESCE((value)::INT, 7) INTO v_max_stock_days
  FROM app_settings WHERE key = 'oa_max_stock_weeks' LIMIT 1;
  v_max_stock_days := v_max_stock_days * 7;

  RETURN QUERY
  SELECT
    p.id AS product_id,
    p.name AS product_name,
    p.sku,
    p.category,
    ps_link.supplier_id,
    s.name AS supplier_name,
    COALESCE(inv.qty_on_hand, 0) AS physical_inventory,
    COALESCE(transit.qty_in_transit, 0) AS confirmed_transits,
    COALESCE(pending.qty_pending, 0) AS pending_customer_orders,
    (COALESCE(inv.qty_on_hand, 0) + COALESCE(transit.qty_in_transit, 0) - COALESCE(pending.qty_pending, 0)) AS net_inventory,
    COALESCE(dem.avg_demand, 0) AS avg_daily_demand,
    CASE
      WHEN COALESCE(dem.avg_demand, 0) > 0
      THEN (COALESCE(inv.qty_on_hand, 0) + COALESCE(transit.qty_in_transit, 0) - COALESCE(pending.qty_pending, 0)) / dem.avg_demand
      ELSE 9999
    END AS days_of_supply,
    CASE
      WHEN COALESCE(dem.avg_demand, 0) <= 0 THEN 'sin_demanda'
      WHEN (COALESCE(inv.qty_on_hand, 0) + COALESCE(transit.qty_in_transit, 0) - COALESCE(pending.qty_pending, 0)) / dem.avg_demand < v_min_stock_days THEN 'critical'
      WHEN (COALESCE(inv.qty_on_hand, 0) + COALESCE(transit.qty_in_transit, 0) - COALESCE(pending.qty_pending, 0)) / dem.avg_demand < 5 THEN 'low'
      WHEN (COALESCE(inv.qty_on_hand, 0) + COALESCE(transit.qty_in_transit, 0) - COALESCE(pending.qty_pending, 0)) / dem.avg_demand <= v_max_stock_days THEN 'optimal'
      ELSE 'excess'
    END AS buffer_status,
    COALESCE(p.is_export, false) AS is_export,
    p.supplier_origin
  FROM products p
  INNER JOIN product_suppliers ps_link ON ps_link.product_id = p.id
  INNER JOIN suppliers s ON s.id = ps_link.supplier_id
  LEFT JOIN LATERAL (
    SELECT SUM(id.quantity_on_hand) AS qty_on_hand
    FROM inventory_daily id
    WHERE id.product_id = p.id
      AND id.snapshot_date = (SELECT MAX(snapshot_date) FROM inventory_daily)
      AND (p_warehouse_id IS NULL OR id.warehouse_id = p_warehouse_id)
  ) inv ON true
  LEFT JOIN LATERAL (
    SELECT SUM(pol.quantity - COALESCE(pol.received_qty, 0)) AS qty_in_transit
    FROM purchase_order_lines pol
    JOIN purchase_orders po ON po.id = pol.order_id
    WHERE pol.product_id = p.id
      AND po.state IN ('purchase', 'locked')
      AND (pol.quantity - COALESCE(pol.received_qty, 0)) > 0
  ) transit ON true
  LEFT JOIN LATERAL (
    SELECT SUM(sol.quantity - COALESCE(sol.delivered_qty, 0)) AS qty_pending
    FROM sale_order_lines sol
    JOIN sale_orders so ON so.id = sol.order_id
    WHERE sol.product_id = p.id
      AND so.state IN ('sale', 'done')
      AND (sol.quantity - COALESCE(sol.delivered_qty, 0)) > 0
  ) pending ON true
  LEFT JOIN LATERAL (
    SELECT AVG(dd.quantity_sold) AS avg_demand
    FROM demand_daily dd
    WHERE dd.product_id = p.id
      AND dd.is_censored = false
      AND dd.demand_date >= (SELECT MAX(snapshot_date) FROM inventory_daily) - INTERVAL '30 days'
  ) dem ON true
  WHERE p.is_active = true
    AND COALESCE(dem.avg_demand, 0) > 0
    AND (p_supplier_ids IS NULL OR ps_link.supplier_id = ANY(p_supplier_ids))
  ORDER BY days_of_supply ASC;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 3. UPDATED HOT LIST — Now includes is_export and warehouse filter
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_hot_list(
  p_supplier_ids INT[] DEFAULT NULL,
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  product_id INT,
  product_name VARCHAR,
  sku VARCHAR,
  category VARCHAR,
  supplier_id INT,
  supplier_name VARCHAR,
  net_inventory NUMERIC,
  avg_daily_demand NUMERIC,
  days_of_supply NUMERIC,
  urgency_qty NUMERIC,
  is_export BOOLEAN,
  supplier_origin VARCHAR
) AS $$
DECLARE
  v_min_stock_days INT;
BEGIN
  SELECT COALESCE((value)::INT, 3) INTO v_min_stock_days
  FROM app_settings WHERE key = 'oa_min_stock_days' LIMIT 1;

  RETURN QUERY
  SELECT
    ni.product_id,
    ni.product_name,
    ni.sku,
    ni.category,
    ni.supplier_id,
    ni.supplier_name,
    ni.net_inventory,
    ni.avg_daily_demand,
    ni.days_of_supply,
    GREATEST(0, (v_min_stock_days * ni.avg_daily_demand) - ni.net_inventory) AS urgency_qty,
    ni.is_export,
    ni.supplier_origin
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
  WHERE ni.buffer_status = 'critical'
  ORDER BY ni.days_of_supply ASC;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 4. UPDATED HOLD LIST — Export products flagged as non-cancellable
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_hold_list(
  p_supplier_ids INT[] DEFAULT NULL,
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  product_id INT,
  product_name VARCHAR,
  sku VARCHAR,
  category VARCHAR,
  supplier_id INT,
  supplier_name VARCHAR,
  net_inventory NUMERIC,
  avg_daily_demand NUMERIC,
  days_of_supply NUMERIC,
  excess_qty NUMERIC,
  excess_days NUMERIC,
  is_export BOOLEAN,
  supplier_origin VARCHAR,
  cancellable BOOLEAN
) AS $$
DECLARE
  v_max_stock_days INT;
BEGIN
  SELECT COALESCE((value)::INT, 7) INTO v_max_stock_days
  FROM app_settings WHERE key = 'oa_max_stock_weeks' LIMIT 1;
  v_max_stock_days := v_max_stock_days * 7;

  RETURN QUERY
  SELECT
    ni.product_id,
    ni.product_name,
    ni.sku,
    ni.category,
    ni.supplier_id,
    ni.supplier_name,
    ni.net_inventory,
    ni.avg_daily_demand,
    ni.days_of_supply,
    GREATEST(0, ni.net_inventory - (v_max_stock_days * ni.avg_daily_demand)) AS excess_qty,
    GREATEST(0, ni.days_of_supply - v_max_stock_days) AS excess_days,
    ni.is_export,
    ni.supplier_origin,
    -- Export products CANNOT be cancelled once ordered
    NOT COALESCE(ni.is_export, false) AS cancellable
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
  WHERE ni.buffer_status = 'excess'
  ORDER BY ni.days_of_supply DESC;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 5. UPDATED RECEPTION SATURATION — Multi-warehouse support
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_reception_saturation(
  p_date DATE DEFAULT CURRENT_DATE,
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  scheduled_date DATE,
  total_trucks INT,
  total_unload_hours NUMERIC,
  available_dock_hours NUMERIC,
  saturation_pct NUMERIC,
  is_saturated BOOLEAN,
  warehouse_name VARCHAR,
  trucks JSONB
) AS $$
BEGIN
  RETURN QUERY
  WITH target_warehouse AS (
    SELECT
      w.id AS wid,
      w.name AS wname,
      COALESCE(wc.num_docks, 1) AS num_docks,
      EXTRACT(HOUR FROM (COALESCE(wc.working_hours_end, '00:00'::TIME) - COALESCE(wc.working_hours_start, '06:00'::TIME))
        + CASE WHEN wc.working_hours_end <= wc.working_hours_start THEN INTERVAL '24 hours' ELSE INTERVAL '0' END
      ) AS work_hours
    FROM warehouses w
    LEFT JOIN warehouse_config wc ON wc.warehouse_id = w.id
    WHERE (p_warehouse_id IS NULL OR w.id = p_warehouse_id)
      AND w.id IN (1, 2, 3, 4)
  ),
  day_schedule AS (
    SELECT
      rs.scheduled_date,
      tw.wid,
      tw.wname,
      tw.num_docks,
      tw.work_hours,
      COUNT(*) AS truck_count,
      SUM(COALESCE(rs.estimated_unload_hours, 2)) AS total_hours,
      jsonb_agg(jsonb_build_object(
        'id', rs.id,
        'supplier_id', rs.supplier_id,
        'unit_type', rs.unit_type,
        'scheduled_time', rs.scheduled_time,
        'estimated_hours', COALESCE(rs.estimated_unload_hours, 2),
        'status', rs.status,
        'priority', rs.priority,
        'hot_list_products', rs.hot_list_products
      ) ORDER BY rs.priority DESC, rs.scheduled_time) AS trucks_json
    FROM reception_schedule rs
    CROSS JOIN target_warehouse tw
    WHERE rs.scheduled_date = p_date
      AND rs.status NOT IN ('cancelled', 'rescheduled')
      AND (p_warehouse_id IS NULL OR rs.warehouse_id = p_warehouse_id)
    GROUP BY rs.scheduled_date, tw.wid, tw.wname, tw.num_docks, tw.work_hours
  )
  SELECT
    ds.scheduled_date,
    ds.truck_count::INT AS total_trucks,
    ds.total_hours AS total_unload_hours,
    (ds.num_docks * ds.work_hours)::NUMERIC AS available_dock_hours,
    CASE
      WHEN (ds.num_docks * ds.work_hours) > 0
      THEN ROUND((ds.total_hours / (ds.num_docks * ds.work_hours)) * 100, 1)
      ELSE 0
    END AS saturation_pct,
    ds.total_hours > (ds.num_docks * ds.work_hours) AS is_saturated,
    ds.wname::VARCHAR AS warehouse_name,
    ds.trucks_json AS trucks
  FROM day_schedule ds;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 6. UPDATED SUPPLIER SEMAPHORE — Now includes is_export and warehouse filter
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_supplier_semaphore(
  p_supplier_ids INT[] DEFAULT NULL,
  p_warehouse_id INT DEFAULT NULL
)
RETURNS TABLE(
  product_id INT,
  product_name VARCHAR,
  sku VARCHAR,
  supplier_id INT,
  supplier_name VARCHAR,
  net_inventory NUMERIC,
  avg_daily_demand NUMERIC,
  days_of_supply NUMERIC,
  semaphore VARCHAR,
  semaphore_reason TEXT,
  is_export BOOLEAN,
  supplier_origin VARCHAR
) AS $$
DECLARE
  v_min_stock_days INT;
  v_max_stock_days INT;
BEGIN
  SELECT COALESCE((value)::INT, 3) INTO v_min_stock_days
  FROM app_settings WHERE key = 'oa_min_stock_days' LIMIT 1;

  SELECT COALESCE((value)::INT, 7) INTO v_max_stock_days
  FROM app_settings WHERE key = 'oa_max_stock_weeks' LIMIT 1;
  v_max_stock_days := v_max_stock_days * 7;

  RETURN QUERY
  SELECT
    ni.product_id,
    ni.product_name,
    ni.sku,
    ni.supplier_id,
    ni.supplier_name,
    ni.net_inventory,
    ni.avg_daily_demand,
    ni.days_of_supply,
    CASE
      WHEN ni.days_of_supply < v_min_stock_days THEN 'rojo'
      WHEN ni.days_of_supply < 5 THEN 'amarillo'
      WHEN ni.days_of_supply <= v_max_stock_days THEN 'verde'
      ELSE 'hold'
    END AS semaphore,
    CASE
      WHEN ni.days_of_supply < v_min_stock_days
        THEN 'Necesidad de Ampliación de OA — Pedido Extraordinario por aceleración de demanda'
      WHEN ni.days_of_supply < 5
        THEN 'Sugerencia de adelantar despachos de la próxima semana'
      WHEN ni.days_of_supply <= v_max_stock_days
        THEN 'Orden Abierta fluyendo según lo pactado'
      ELSE CASE
        WHEN COALESCE(ni.is_export, false)
          THEN 'Exceso de inventario — Exportación: no se puede pausar despacho en tránsito'
          ELSE 'Detener despachos — exceso de inventario sobre buffer de seguridad'
        END
    END AS semaphore_reason,
    ni.is_export,
    ni.supplier_origin
  FROM rpc_oa_net_inventory(p_supplier_ids, p_warehouse_id) ni
  ORDER BY
    CASE
      WHEN ni.days_of_supply < v_min_stock_days THEN 1
      WHEN ni.days_of_supply < 5 THEN 2
      WHEN ni.days_of_supply <= v_max_stock_days THEN 3
      ELSE 4
    END,
    ni.days_of_supply ASC;
END;
$$ LANGUAGE plpgsql;


-- ============================================================================
-- 7. ALERTS SUMMARY — For login banner
-- ============================================================================
CREATE OR REPLACE FUNCTION rpc_oa_alerts_summary()
RETURNS TABLE(
  hot_count BIGINT,
  hold_count BIGINT,
  hold_export_count BIGINT,
  reception_saturated BOOLEAN,
  reception_trucks_today INT,
  warehouse_alerts JSONB
) AS $$
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(*) FROM rpc_oa_hot_list(NULL, NULL)) AS hot_count,
    (SELECT COUNT(*) FROM rpc_oa_hold_list(NULL, NULL)) AS hold_count,
    (SELECT COUNT(*) FROM rpc_oa_hold_list(NULL, NULL) h WHERE h.is_export = true) AS hold_export_count,
    COALESCE((SELECT bool_or(rs.is_saturated) FROM rpc_oa_reception_saturation(CURRENT_DATE, NULL) rs), false) AS reception_saturated,
    COALESCE((SELECT SUM(rs.total_trucks)::INT FROM rpc_oa_reception_saturation(CURRENT_DATE, NULL) rs), 0) AS reception_trucks_today,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'warehouse_name', ws.warehouse_name,
        'alert_level', ws.alert_level,
        'saturation_pct', ws.saturation_pct
      )) FROM rpc_oa_warehouse_space(NULL) ws WHERE ws.alert_level IN ('amarillo', 'rojo')),
      '[]'::JSONB
    ) AS warehouse_alerts;
END;
$$ LANGUAGE plpgsql;
