...
> **⚡ Active workstream since 2026-07-23 — COMPRAS / Wilmer xlsx elimination (client-committed: live by Aug 31).** Not tracked in the tables below; its own trackers own the detail: [docs/compras/WILMER_COMPRAS_PROGRESS.md](docs/compras/WILMER_COMPRAS_PROGRESS.md) (xlsx replica — page shipped, engine 99.85% parity) and [docs/compras/REABASTECIMIENTO_LIVE_PROGRESS.md](docs/compras/REABASTECIMIENTO_LIVE_PROGRESS.md) (live-Odoo endpoint — schema applied, sync in design). Open questions: [docs/compras/OPEN_QUESTIONS.md](docs/compras/OPEN_QUESTIONS.md). **2026-08-06 call extracted 08-11** ([docs/compras/RESPUESTAS_WILMER_2026-08-06.md](docs/compras/RESPUESTAS_WILMER_2026-08-06.md)): semáforo aprobado (3/7/30), velocidad = ordenado − {cotización, cot. enviada, cancelado}, 2Z11→San José, 5DEP fuera de venta; nuevos asks: **export Excel→Odoo masivo (I1)** + formulario forecast comercial spec'd (I2, Raquel gatekeeper); pendientes de re-preguntar: múltiplos de despacho, mecánica Carvajal (I6 General/patio: **resuelta 08-11**, transcript más reciente gobierna — sync excluye 5DEP/Entrada del General + tooltips include/exclude en todos los títulos del vivo; reunión Raquel **pospuesta**, sin fecha). **Delivery phase 2026-08-11:** live credentials to Wilmer/Alexis + in-app bug-report widget shipped ([docs/feedback/](docs/feedback/BUG_REPORT_DESIGN.md)) — ✅ Alexis user created 2026-08-11: **`comprasinternacionales@airefill.app`**, role `inventario`, display name "Alexis" (login verified). Wilmer's `compras@airefill.app` password rotated same day. Both on temp passwords — rotate/have them change post-validation (`/update-password`).
...
> **⚡ Sesiones 2026-08-20 extraídas (2026-08-21).** Dos llamadas usables de cuatro grabaciones. **Wilmer** → [docs/compras/RESPUESTAS_WILMER_2026-08-20.md](docs/compras/RESPUESTAS_WILMER_2026-08-20.md) (serie I8–I14; asks W11–W14 + O11): la cadena **San José Villanueva → Zacapa → Petén** (*"es una cadenita"*) — pide Zacapa y Petén **separados Y sumados**, y ⚠️ **medido 08-21: la app los fusiona** en una sola bodega `'Zacapa-Petén'`; **el export se duplicó en alcance** (formato del proveedor por semana × bodega, definido por el *robot* de Carvajal, **más** tres OC de Odoo, una por bodega); **alerta de tendencia creciente** (*"que me tire un signo de advertencia… la tendencia es incrementar los últimos tres meses"*) — ⚠️ **adyacente al hallazgo del estacional `h`, no su solución**: notifica en vez de corregir, y dispara por tendencia mientras el defecto es de cobertura; drill-down de tránsito por ETA pedido por **Mario** (mismo build que A2 de Alexis), que está entrando con la credencial de Wilmer. **P11 (mecánica Carvajal) quedó respondida sin preguntarla.** **Alexis** → [docs/docs-alexis/MANIFEST.md](docs/docs-alexis/MANIFEST.md) §1f: **N12 resuelto** — la bolsa poliseda se factura por **peso** (`KGM`) y se compra por **millar**; el conteo de bultos viaja en la cola de la descripción, no en la cantidad; regla `cantidad = BLTS × rollos_por_bulto` con la tablita del cliente (migración `20260820000004`, ⚠️ sin commitear al 08-21). ❌ **Las otras dos grabaciones no se transcribieron**: 132 min de la reunión de forecast produjeron 585 palabras (fallo del transcriptor, no reunión corta) — hay que recuperar el audio; ver [docs/20260820/README.md](docs/20260820/README.md).
> **⚡ Delivery-phase asks tracker (new 2026-08-19):** every explicit request from Wilmer, Alexis, David, Luis Roberto, Raquel, Mario and gerencia that is **not yet available in the app**, plus the append-only log of what has been closed since delivery began, now lives in one place: `docs/DELIVERY_OPEN_REQUESTS_2026-08-19.md` (gitignored with the rest of `docs/`). Stable ids: `W*` Wilmer · `A*` Alexis · `O*` others · `I*` infrastructure. This file stays the index for tracks D/M/H/F/C; that one is the index for **who asked for what**.
