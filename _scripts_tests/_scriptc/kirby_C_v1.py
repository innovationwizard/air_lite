...
# Configurable paths (update or place files in script dir)
...
def parse_txt(file_path):
    try:
        ...
        print(f"Error: {file_path} not found. Place in dir or update path.")
        return pd.DataFrame()

def clean_clients(file_path):
    try:
        df = pd.read_csv(file_path, header=None, names=['client_id', 'client_name'], on_bad_lines='warn')
        orig = len(df)
...
def clean_returns(file_path):
    try:
        df = pd.read_csv(file_path, header=None, names=['amount', 'qty', 'price', 'client_id', 'sku', 'product', 'date', 'market', 'nc_code', 'reason', 'col10', 'col11'], on_bad_lines='warn')
        orig = len(df)
        df = df.drop_duplicates()
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
        df = df.dropna(subset=['date'])
        df[['amount', 'qty', 'price']] = df[['amount', 'qty', 'price']].apply(pd.to_numeric, errors='coerce')
        df['sku'] = df['sku'].astype(str).str.zfill(9)
        df['source'] = 'returns'
...
def clean_sales(file_path):
    try:
        df = pd.read_csv(file_path, header=None, names=['col0', 'qty', 'price', 'unit_price', 'col4', 'col5', 'client_id', 'sku', 'product', 'date', 'col9', 'market', 'ff_code', 'col13', 'col14', 'col15'], on_bad_lines='warn')
        orig = len(df)
        df = df.drop_duplicates()
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
        df = df.dropna(subset=['date'])
...
def clean_excel(file_path):
    try:
        ...
        if not pd.api.types.is_datetime64_any_dtype(df_sales['Fecha y hora de venta']):
            df_sales['Fecha y hora de venta'] = pd.to_datetime(df_sales['Fecha y hora de venta'], origin='1899-12-30', unit='D', errors='coerce')
        ...
        # Sheet 3: Returns Monthly (unpivot)
        df_returns_monthly = pd.read_excel(xls, sheet_name=3, header=1)
        orig_returns = len(df_returns_monthly)
        months = ['octubre 2024', 'noviembre 2024', 'diciembre 2024', 'enero 2025', 'febrero 2025', 'marzo 2025', 'abril 2025', 'mayo 2025', 'junio 2025', 'julio 2025', 'agosto 2025', 'septiembre 2025']
        melt_cols = [col for col in df_returns_monthly.columns if any(m in col for m in months)]
        df_returns_monthly = df_returns_monthly.melt(id_vars=['Producto', 'Vendedor', 'Cliente'], value_vars=melt_cols, var_name='metric_month', value_name='value')
        ...
        if not pd.api.types.is_datetime64_any_dtype(df_inv['Fecha y hora']):
            df_inv['Fecha y hora'] = pd.to_datetime(df_inv['Fecha y hora'], origin='1899-12-30', unit='D', errors='coerce')
        ...
        if not pd.api.types.is_datetime64_any_dtype(df_purch['Fecha de orden de compra']):
            df_purch['Fecha de orden de compra'] = pd.to_datetime(df_purch['Fecha de orden de compra'], origin='1899-12-30', unit='D', errors='coerce')
        if not pd.api.types.is_datetime64_any_dtype(df_purch['Fecha de esperada']):
            df_purch['Fecha de esperada'] = pd.to_datetime(df_purch['Fecha de esperada'], origin='1899-12-30', unit='D', errors='coerce')
...
def insert_to_postgres(df, table_name, conn_params):
    ...
    col_defs = ', '.join([f'"{col}" TEXT' for col in cols])  # All text for simplicity; adjust types as needed
...
def main():
    ...
    if df_txt.empty: return  # Early exit if key file missing
    ...
    if all(d.empty for d in [df_clients, df_returns, df_sales, df_excel]):
