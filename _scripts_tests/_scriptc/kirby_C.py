...
from psycopg2.extras import execute_batch  # optional but fast
...
def insert_to_postgres(df, table_name, conn_params):
    ...
    # 1) CREATE TABLE safely (handles spaces, % signs, accents)
    ...
    # 2) COPY (fast + robust; no placeholder parsing)
    # Ensure string dtype to avoid binary adapters surprises
    df_to_copy = df.astype(str)

    csv_buf = io.StringIO()
    df_to_copy.to_csv(csv_buf, index=False)  # header included
    csv_buf.seek(0)
    ...
    cur.copy_expert(copy_sql.as_string(cur), csv_buf)

    conn.commit()
    cur.close()
    conn.close()
    print(f"Data inserted into Postgres table '{table_name}' (rows: {len(df_to_copy)})")
