...
# -*- coding: utf-8 -*-
"""
kirby_C.py — SSOT builder with full reconciliation
--------------------------------------------------
Features
- Robust ingestion of:
  * CSVs: Venta 15 al 24.csv, devolucion 15 al 24.csv, clientes 15 al 24.csv (headerless)
  * Excel (optional): Envio de datos a Condor.xlsx (uses openpyxl)
- Normalization:
  * sku → 9-char zero-padded text
  * client names → merged and normalized
  * placeholder labels: ASK_* and ASK_UNIT_OR_PACK_qty
- SQLite outputs with performance pragmas + chunked to_sql
- Materialized staging with indexes for join keys
- Name→ID mapping for Excel "Cliente" (names) → numeric client_id (from clientes)
  * optional alias file: --client-alias path.csv (columns: excel_name,client_id), applied before normalized match
- Deterministic DQ join with configurable tolerance: --dq-mode {exact,5min,60min,day}
- Observations→Matches→Events model:
  * sales_observation: ALL raw rows with provenance (csv/excel)
  * sales_match: links csv↔excel potential duplicates (per tolerance)
  * sales_event: reconciled events with per-SKU policy (excel/csv/mean/max/sum)
  * recon rules optional CSV: --recon-rules rules.csv (columns: sku,strategy)
- Summaries:
  * dq_summary (dup_equal/conflict_qty and left-out % relative to Excel)
  * sales_event_summary (paired/conflict rates)
- Exports key CSVs to the output folder.

Usage
-----
python kirby_C.py --input . --output ./out \
  --dq-mode 5min \
  [--client-alias ./client_alias.csv] \
  [--recon-rules ./recon_rules.csv]
...
def apply_client_alias_and_map(con, client_alias_csv: str|None):
    ...
    if client_alias_csv and Path(client_alias_csv).exists():
        ...
        if not {'excel_name','client_id'}.issubset(set(alias_df.columns)):
            raise ValueError("client_alias CSV must have columns: excel_name,client_id")
        ...
        alias_join = "LEFT JOIN client_alias a ON a.excel_name_norm = e.client_name_norm"

    con.executescript(f"""
    DROP TABLE IF EXISTS stg_sales_excel_mapped;
    CREATE TABLE stg_sales_excel_mapped AS
    SELECT e.ts, e.sku,
           COALESCE(a.client_id, d.client_id, e.cliente_raw) AS client_id,
...
def main():
    ap = argparse.ArgumentParser(description="Build SSOT with reconciliation (kirby_C)")
