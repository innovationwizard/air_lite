...
**Horizon 2:** Alexis formal demo — date TBD (see Open Question #8)
...
- **Alexis (Gerente General) is NOT convinced.** He is the single decision maker. David: *"sin eso, aunque queramos ayudar a los demás, no nos va a servir... él es el tomador de decisiones máximo"* (lines 355–363).
- Alexis has a buying committee he reports to ("la junta directiva"). David needs ammunition to defend the project upward (lines 262–266).
- Alexis is explicitly skeptical, not hostile. He wants to see numbers that prove the tool is more atinado than his current compradores before he signs off on a 6-month implementation.
- David's own next move: he will spend tomorrow morning (2026-04-23 09:30) sanity-checking the tool's projections against reality **before** scheduling the Alexis meeting (lines 738–778). Jorge is the demo operator; David is the first filter.

---

## 2. WHAT ALEXIS NEEDS TO SEE (verbatim, with line refs)

Three asks, ranked by Alexis's own priority.
...
Alexis's own framing (lines 502–531): *"cuánto me representó en dinero esa proyección... cuánto tenía que haber gastado... cuánto tendría que vender... tuve 25 mil extras... cuánto tendría que haber ganado en base a tu proyección."*

### Ask 3 — "Si se pudiera sería excelente" — Cubicaje (lines 80–98, 539–608)

**Not a blocker. Explicit nice-to-have.**

Alexis's frame: *"sería excelente, solo para que tones en cuenta"* (line 541–544). He wants to know:
...
4. Only after that does the Alexis meeting get scheduled.

**The point of the 9:30 meeting is calibration measurement, not persuasion.** Jorge needs to know exactly where the tool over- and under-shoots **before** Alexis sees anything. The tool must be honest by design — if we curate the SKU list to only winners, David will catch it.
...
- **Holdout-framing narrative.** Current UI presents savings, not calibration. For Alexis we need the explicit statement: *"El modelo vio datos hasta 2025-01-01. Predijo X. Lo que realmente pasó fue Y. Los compradores compraron Z."*
...
- **`backtest_savings` exists but is aggregate and partially broken** (storage flat 0%, stockout flat 80%, rotation flat 53.85% — per refactor plan §2.4). We cannot reuse it directly for Alexis; we must derive fresh per-SKU monetary numbers.
...
- **Prod-DB verified (2026-04-22):** `backtest_runs` has IDs 57–70, each `products_modeled=100`. Runs **58, 59, 60, 61** are the Feb/Mar/Apr/May 2025 predictions Alexis asked about (training ends 2025-01-31, -02-28, -03-31, -04-30 respectively; `prediction_month` = 2025-02-01, -03-01, -04-01, -05-01). `purchase_orders.order_date` range: 2024-10-01 → 2026-03-24, fully covers all 14 cycles.
...
- Cycle selector (dropdown of the 14 runs, IDs 57–70). **Default: run 58** (predicts Feb 2025 from a 2025-01-31 training cutoff — the first month Alexis asked about). Recommended demo sequence: 58 → 59 → 60 → 61 (Alexis's Feb/Mar/Apr/May 2025 sequence).
...
- **"SKU no modelado" handling:** If Alexis asks about a SKU that is NOT in the 100 `products_modeled` for the selected run, the UI shows *"Este SKU no fue modelado en este ciclo. Está entre los 615 SKUs elegibles no priorizados. Podemos agregarlo en el próximo ciclo."* — not an error, not silence.
...
This is the holdout narrative Alexis wants. It is literally what Prophet does in the backtest, but the UI does not say it today.
...
- New RPC `rpc_cubicaje_projection(p_run_id INT)` returning:
  ...
  - The furgón count is derived from the fleet average — no hardcoded constant. E.g. if Peten overflow = 180 m³ and the 3 Peten trucks average 25.55 m³, that's ≈7 Peten deliveries. This is the honest answer Alexis asked for.
...
**Cut point:** If `products.volume` is too sparse (<50% populated for the bodega's top-demand SKUs), show a clear disclaimer instead of the chart and skip this section in the demo. If the fleet data isn't ingested (Q14 is "no"), seed the 29 rows directly from the exploration doc — the numbers are static enough for the Alexis demo and the ingestion can follow post-approval.
...
This is Ask 2's ≈ 30-second-answer summary. If Alexis leaves the meeting remembering one number, this is the one.

#### P1.3 — Archived-code handling audit (≈1h)

Per handover doc §1 and silo doc insight §2, archived codes in Odoo destroy sales history. If `/gerencia/validacion` shows a top-moving SKU with zero `actual_demand` because the code was archived mid-period, Alexis will flag it as a data bug and question the whole tool. **Before the Alexis demo**, run a query:
...
On the call, David asked *"subiste la API a producción?"* — Jorge: *"No, a producción no"* (lines 670–674). Alexis will ask the same question.
...
### P2 — Post-Alexis approval (6-month scope per Jorge's line 293–302 estimate)

Do NOT start any of these before Alexis says yes.
...
Pulled directly from the feedback transcript and the silo doc. Listed so Jorge can point here when Alexis asks "¿por qué no está X?":

- Slot-level or rack-level warehouse mapping (Alexis mentioned it as a general future want; silo doc §MARIO "NOT feasible").
- Container scheduling optimizer (lines 149–163 — "movimientos ya más avanzados"; Jorge explicitly pushed to Sprint 2+).
- Rebuilding Wilmer/Mario silo views — the refactor from this morning stands. Alexis's validation page is additive, not a replacement.
...
- Any feature Alexis did NOT name in the transcript. This plan is scoped to what he actually asked for.
...
| Archived-code contamination inflates the "tool wrong" signal | **High** | David catches it, or worse, Alexis catches it | Run the audit in P1.3 BEFORE the David meeting, not after. |
...
| Jorge oversells calibration after one good cycle, David's outside data crushes it | Medium | Credibility loss with David, cascades to Alexis | Run P0.3 (the paper cheat-sheet) before the 9:30. Go in knowing the worst misses, don't be surprised by them. |
| Localhost demo for Alexis | **High** | *"Es que todavía no está en producción"* → same skepticism Wilmer said Alexis already has about "mucho tiempo, necesito respuesta" (lines 46–59) | Ship P1.4 before the Alexis meeting. If deployment isn't feasible in the window, get written sign-off that localhost is acceptable — do not assume. |
| Snapshot is 2026-03-03, Alexis wants Feb–May 2025 projections | Low (data covers it) | None if the cycle window maps correctly | Confirm the cycle window. Pick a cycle whose training cutoff is 2025-01-31 or earlier. See Open Question #3. |
...
| Data freshness question from Alexis | **High** | *"Los datos son del 3 de marzo"* → honest acknowledgment, pre-empt | The holdout banner from P0.2 pre-empts this. Frame the snapshot age as a **feature** of the holdout: we're predicting past-dated months on purpose, not hiding fresh data. |
...
3. **Cycle window aligning with Feb–May 2025.** ✅ **RESOLVED 2026-04-22 via direct prod probe.** `backtest_runs` (not `backtest_cycles` — that table does not exist) has 14 rolling-origin holdouts, IDs 57–70, each training on `2024-10-01 → month-end-N` and predicting month N+1. The four that match Alexis's ask:
...
11. **`/compras/programacion` redirect and Wilmer's OA access.** Per the refactor plan this morning, are Wilmer and Mario's sidebars actually verified working post-merge? If any regression is open, fix-forward takes priority over new work because Alexis will likely click around during his demo.

12. **Who presents to Alexis?** David (lines 762–766): *"te voy a reunir contigo porque tú eres quien lo va a presentar"*. Confirm Jorge is the presenter so tomorrow's rehearsal is meaningful.
...
15. **"SKU no modelado" edge case.** Prod probe (2026-04-22) confirms each backtest run has `products_modeled = 100` out of 715 eligible. If Alexis asks to see "faros" specifically and faros aren't in the 100, the tool must say so cleanly — not hide, not error. Covered in §5 P0.1 Page spec. But: **which 100 SKUs are in each run?** Is the selection stable across runs 57–70, or does it vary? If stable, the 100 are probably the top ABC-movers — safe bet Alexis's examples are included. If unstable, we need to pre-verify run 58's SKU list before the 9:30. Quick verification: `SELECT COUNT(DISTINCT product_id) FROM backtest_results WHERE run_id = 58;` should equal 100, and `SELECT sku FROM products WHERE id IN (...)` will tell us which.

---

## 10. THE ONE-LINER FOR ALEXIS

The entire tool's pitch to Alexis, written as one sentence Jorge can actually say out loud:
...
| `fleet.vehicle` was access-denied at 07:48, granted at 10:10 same day | — | Evidence that access requests get approved — useful for the Alexis-meeting promise that "data barriers unblock within hours" |
...
**Backtest cycles covering Alexis's Feb–May 2025 ask:**

| Run ID | training_start | training_end | prediction_month | Alexis mention |
|---|---|---|---|---|
...
Alexis's timestamp ask (*"comencemos a llevar ese tiempo"*) is not a build problem. It is a **discipline problem with a pre-wired home**. The fields are in Odoo. The bodega team is not filling them. The fastest lever is not air_lite — it is David enforcing a one-line SOP starting Monday:
